...
biexp <- function (conc, time, log.scale=FALSE, baseline=0, tol=1e-09, maxit=500) {
    ...
    if (baseline < 0) {
        stop("pre-dosing value must be greater 0")
    }
    if (baseline > 0) {
        data$conc <- data$conc - baseline
    }
    if (any(data$conc <= 0)) {
    ...
    biexploss <- function(par, fun) {
         sum((fun(conc) - fun(par[1] * exp(-(exp(par[4]) + exp(par[2])) * time) + par[3] * exp(-exp(par[4]) * time)))^2)
    }
    ...
    if(sum.resid == Inf){return(warning("no model evaluated"))}
    switch(type, "4" = {
        sol <- optim(par=start, fn=biexploss, fun=fun, method=c("Nelder-Mead"), 
            control=list(reltol=tol, maxit=maxit))$par
        b1 <- (exp(sol[4]) + exp(sol[2]))
        a1 <- sol[1]
        b2 <- exp(sol[4])
        a2 <- sol[3]
    }, "2" = {
        sol <- optim(par=start, fn=singleloss, fun=fun, method=c("Nelder-Mead"), 
            control=list(reltol=tol, maxit=maxit))$par
        b1 <- exp(sol[2])
        a1 <- sol[1]
        b2 <- b1
