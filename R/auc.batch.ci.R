auc.batch.ci <- function(conc, time, group=NULL, method=c("t", "z", "boott"),  alternative=c("two.sided", "less", "greater"), conf.level=0.95, nsample=1000){
  ...
  parms <- function(w, conc, time, group){
    ...
    if(!is.null(group)){
      ...
      res <- list(var=sum(aucvar1)+sum(aucvar2), est=auc1-auc2, df=sum(aucvar1)^2/sum(aucvar1^2/2)+sum(aucvar2)^2/sum(aucvar2^2/2))
      return(res)
    }else{      
      n<-length(conc[[1]])/length(time[[1]])
      auc<-0
