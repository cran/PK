...
PKNews <- function() file.show(system.file("doc", "NEWS.txt",
package="PK"))
...
###
print.PK <- function (x, digits=max(3, getOption("digits") - 3), ...) {
  ...
  for(m in unique(x$CIs[,'method'])){
    res <- data.frame(format(estimator(x, se = TRUE), digits=digits), 
           ci = paste("(",format(x$CIs[x$CIs[,'method']==m, "lower"], digits = digits), ";", format(x$CIs[x$CIs[,'method']==m, "upper"], digits = digits), ")", sep = ""))
...
plot.PK <- function (x, col=NULL, pch=NULL, main=NULL, xlab="Time", ylab="Concentration", ylim=NULL, xlim=NULL, add=FALSE, ...) {
...
print.PKtest<-function(x,hyp=FALSE,...){
  ...
  if(x$method=='t'){
    ...
    colnames(out) <- c('Statistic','df','p-value',' ')
  }else{
    if(x$method=='z'){
      cat("z-test\n\n")
    }else{
