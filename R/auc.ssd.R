auc.ssd <- function(conc, time, group=NULL, method=c("tang.burke", "bailer", "boott"),  alternative=c("two.sided", "less", "greater"), conf.level=0.95, strata=NULL, nsample=1000, data){
    ...
    resample <- function(w, data, alpha, nsample, bystrata){
        ...
        for(i in 1:nsample){
            ...
            boot.data <- data.frame(time=data$time, conc=boot.conc, group=data$group)
    ...
    if(nlevels(data$strata) == 1){bystrata <- list(data$time, data$group)}
...
z <- rbind(z,resample(w=w, data=data, alpha=alpha, nsample=nsample, bystrata=bystrata))}
    ...
    if(any(method=="tang.burke")){df=c(rep(NA, as.real(length(lower)-1)), v)}
    res <- data.frame(est=est, stderr=sqrt(sum(obsv.parm$var)), lower=lower, upper=upper, df=df)
